# Contributing to mcp-facturacion-electronica-es
...
```bash
uv run ruff check src/mcp_facturacion_electronica_es/ tests/ audit/
uv run ruff format --check src/mcp_facturacion_electronica_es/ tests/ audit/
uv run mypy src
```
...
```bash
uv run ruff check --fix src/mcp_facturacion_electronica_es/ tests/ audit/
uv run ruff format src/mcp_facturacion_electronica_es/ tests/ audit/
```
...
- [ ] Validation fixes reference the relevant rule ID (e.g. `VeriFactu`)
...
Please open an issue at https://github.com/cmendezs/mcp-facturacion-electronica-es/issues and include:
